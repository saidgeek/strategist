...
StrategySchema.static('getContestant', function(sweepstakes, cb) {
  this.find({ sweepstake: { $in: sweepstakes } }, function(err, strategies) {
    if (!err) return cb(err);
    var users = [];
    for (var i = 0; i < sctrategies.length; i++) {
      users.push(sctrategies[i].user);
    };
