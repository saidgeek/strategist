...
exports.select = function(req, res) {
  ...
  Sweepstake.findById(req.params.sweepstake_id, function(err, sweepstake) {
    ...
    Strategy.find({ sweepstake: { $in: sweepstake_ids }, user: req.params.user_id })
      .limit(1)
      .sort('-votes.count')
      .exec(function(err, strategies) {
        ...
        win.save(function(err) {
          ...
          sweepstake.save(function(err) {
            ...
            if (sweepstake.type !== 'GROUP') {
              Strategy.update({ sweepstake: req.params.sweepstake_id, close: false }, { close: true }, { multi: true }).exec();
            };
            Sweepstake.next(req.params.sweepstake_id);
