...
UserSchema
  .pre('save', function(next) {
    ...
    if (!this.isNew && this.isModified('provider.facebook')) {
      jobs.create('log', { 
        action: "USER:ADD:PROVIDER:FACEBOOK",
        data: this
      })
      .priority('high')
      .save();
    ...
    if (!this.isNew && this.isModified('provider.twitter')) {
      jobs.create('log', { 
        action: "USER:ADD:PROVIDER:TWITTER",
        data: this
      })
      .priority('high')
      .save();
...
UserSchema.post('save', function() {
  ...
  if (this.wasNew) {
    ...
    jobs.create('log', { 
      action: "USER:CREATE:"+this.role.toUpperCase(),
      data: this
    })
    .priority('high')
    .save();
